...
import { claimVerifierAdvisoryAttempt, persistVerifierTelemetry } from "./verifier-telemetry.js";
...
describe("verifier telemetry persistence", () => {
  it("claims one durable advisory dispatch and prevents replayed model spend", () => {
    ...
    expect(claimVerifierAdvisoryAttempt(db, input)).toBe(true);
    expect(claimVerifierAdvisoryAttempt(db, input)).toBe(false);
