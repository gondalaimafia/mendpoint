...
import { appendDomainEvent, createDb, insertArtifactManifest, insertPrincipal } from "@mendpoint/db";
...
import { getPostTrainedAdapterStatus, registerPostTrainedAdapter, routePostTrainedAdapterDryRun } from "./post-trained-application.js";
...
function setup() { const root = mkdtempSync(join(tmpdir(), "post-trained-")); roots.push(root); const db = createDb(join(root, "db.sqlite")); dbs.push(db); insertPrincipal(db, { id: "actor", tenantId: "tenant-1", kind: "service", subject: "operator", displayName: "Operator", createdAt: "2026-08-12T12:00:00.000Z" }); const content = JSON.stringify({ encoding: "base64", bytes: ADAPTER_BYTES.toString("base64"), decodedSha256: DIGEST }); const sha256 = createHash("sha256").update(content).digest("hex"); insertArtifactManifest(db, { id: "training-artifact", tenantId: "tenant-1", kind: "post_trained_adapter_artifact", schemaVersion: 1, sha256, mediaType: "application/vnd.mendpoint.post-trained-adapter-bytes+json", sizeBytes: Buffer.byteLength(content), storageRef: "sqlite://training-artifact", content, producerPrincipalId: "actor", createdAt: "2026-08-12T11:00:00.000Z" }); appendDomainEvent(db, { id: "training-completed", tenantId: "tenant-1", schemaVersion: 1, eventType: "post_trained_training.completed", aggregateType: "post_trained_training_job", aggregateId: "training-job-1", actorPrincipalId: "actor", correlationId: "training-1", idempotencyKey: "training-completed-1", payload: { requestDigest: "request", artifactId: "training-artifact", adapterDigest: DIGEST, datasetId: "dataset-1", evaluation: { passed: true }, canary: { passed: true }, completedAt: "2026-08-12T11:00:00.000Z" }, createdAt: "2026-08-12T11:00:00.000Z" }); return db; }
...
describe("post trained application", () => {
  it("durably registers tenant scoped manifest and performs route dry run with fresh consent recheck", () => {
    const db = setup(); let consentReads = 0;
    ...
    const authority = { enabled: true as const, verifyEvidence: () => true, readConsent: (_tenant: string, _dataset: string, stored: PostTrainedConsentSnapshot) => stored };
    ...
    expect(getPostTrainedAdapterStatus(db, "tenant-1", "adapter-1", { enabled: true }).lifecycle.state).toBe("monitored");
    ...
    expect(db.raw.prepare("SELECT COUNT(*) c FROM domain_events WHERE tenant_id = ? AND aggregate_type = 'post_trained_adapter'").get("tenant-1")).toEqual({ c: 1 });
  });
  ...
  it("rejects registration not bound to the exact completed training artifact and current consent", () => {
    ...
    expect(() => registerPostTrainedAdapter(db, registration, { enabled: true, verifyEvidence: () => true, readConsent: (_tenant, _dataset, stored) => stored })).toThrow("post_trained_training_completion_missing");
    expect(() => registerPostTrainedAdapter(db, { ...registration, trainingJobId: "training-job-1" }, { enabled: true, verifyEvidence: () => true })).toThrow("post_trained_consent_authority_missing");
  });
});
