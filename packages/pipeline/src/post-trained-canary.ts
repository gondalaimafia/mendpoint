...
function validateExchange(exchange: PostTrainedCanaryExchange, input: PostTrainedCanaryInput, requestDigest: string, authority: ReturnType<typeof readAuthority>, deps: PostTrainedCanaryDependencies): PostTrainedCanaryResult {
  ...
  if (result.status === "failed") {
    if (!boundedCode(result.code) || !canonicalTime(result.completedAt) || Date.parse(result.completedAt) < Date.parse(input.requestedAt) || !validEvidenceRefs(result.evidenceRefs)) throw new Error("post_trained_canary_result_invalid");
    return result;
  }
  if (result.status !== "completed") return result;
  if (!canonicalTime(result.completedAt) || Date.parse(result.completedAt) < Date.parse(input.requestedAt)) throw new Error("post_trained_canary_result_invalid");
