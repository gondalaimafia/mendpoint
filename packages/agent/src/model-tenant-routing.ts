 ...
 * credential.
 *
 * Precedence of the two substitution mechanisms is explicit: an optional
 * `MENDPOINT_NON_TRAINING_MODEL_PROVIDER` selects WHICH backend (provider,
 * endpoint, key) resolves first, for deployments where the tiers really are
 * different providers; the model-id substitution then runs ON TOP of whatever
 * backend resulted, correcting the model id if it is still a training tier. The
 * provider override picks the backend; the model-id substitution is the final,
 * lighter safety net on that backend's model id.
 *
 ...
 * deployments where the tiers really are different providers; when set it picks
 * the backend first, then this substitution runs on top of the model it
 * resolved (see the precedence note in this file's header).
 *
