...
describe("Warden runtime effect execution", () => {
  ...
  it("seals and exactly replays an authority-bound terminal outcome", async () => {
    const journal = memoryJournal();
    const execution = await openWardenRuntimeExecution({
      binding,
      journal,
    ...
    const outcome = Object.freeze({
      ...
      candidateManifestDigest: digest("candidate-manifest"),
      evidenceDigest: digest("evidence"),
      changedPathsDigest: digest("src/client.ts"),
    });

    const first = await execution.finalize(outcome);
    const replay = await execution.finalize(outcome);

    expect(first).toEqual(replay);
    expect(first.envelope.payload.phase).toBe("terminal");
    expect(execution.state()).toMatchObject({
      phase: "terminal",
      privateState: { terminalOutcome: outcome },
      pendingEffect: { kind: "none" },
    });
    await expect(execution.finalize({ ...outcome, evidenceDigest: digest("other") }))
      .rejects.toThrow("warden_runtime_terminal_conflict");
    await expect(execution.runEffect({
      kind: "artifact",
      slot: "after-terminal",
      request: { path: "late.json" },
      executor: {
        reconcile: async () => ({ status: "not_started" as const }),
        executeIdempotent: async () => ({ locator: "artifact:late" }),
      },
      validateResult: locatorResult,
      apply: (state) => state,
    })).rejects.toThrow("warden_runtime_terminal_sealed");
  });

  it("reconciles a lost terminal checkpoint response and rejects a stale finalizer", async () => {
    const journal = memoryJournal();
    const execution = await openWardenRuntimeExecution({
      binding,
      journal,
    ...
    journal.loseResponseAfterGeneration(2);

    await expect(execution.finalize(outcome)).resolves.toMatchObject({
      envelope: { payload: { generation: 2, phase: "terminal" } },
    });

    journal.setLease(2);
    await expect(execution.finalize(outcome)).rejects.toThrow("warden_runtime_effect_lease_stale");
  });

  it("holds exclusive controller ownership while terminalizing", async () => {
    ...
    const journal: WardenCheckpointJournal = {
      async read(value) {
        ...
        return await baseJournal.read(value);
      },
      async compareAndSwap(value) {
        return await baseJournal.compareAndSwap(value);
      },
    };
    const execution = await openWardenRuntimeExecution({
      binding,
    ...
    let executeCount = 0;
    blockRead = true;
    const finalizing = execution.finalize(outcome);
    await readStarted;

    try {
      await expect(execution.runEffect({
        kind: "artifact",
        slot: "during-terminalization",
        request: { path: "late.json" },
        executor: {
          reconcile: async () => ({ status: "not_started" as const }),
          executeIdempotent: async () => {
        ...
        validateResult: locatorResult,
        apply: (state) => state,
      })).rejects.toThrow("warden_runtime_effect_concurrent");
    } finally {
      releaseRead();
    }
    await finalizing;
