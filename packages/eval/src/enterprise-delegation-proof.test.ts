  ...
  const evidence: Omit<DelegatedPrTrialEvidence, "attestation"> = {
    ...
    verification: {
      ...
      failToPass: { artifact: fail, authorityId: contract.verification.authorityId, authorityDigest: contract.verification.authorityDigest,
        ...
        failingCheckIdentities: { status: "not_observed", reason: "check_identities_not_parsed_from_runner_output" },
        sandboxBackend: "fly-sandbox", logsDigest: digest(`fail-logs-${number}`) },
      passToPass: { artifact: pass, authorityId: contract.verification.authorityId, authorityDigest: contract.verification.authorityDigest,
        ...
        failingCheckIdentities: { status: "not_observed", reason: "check_identities_not_parsed_from_runner_output" },
        sandboxBackend: "fly-sandbox", logsDigest: digest(`pass-logs-${number}`) },
    ...
    delivery: {
      ...
      changedPaths: ["src/client.ts"], treeDigest: candidateTreeDigest, remoteTreeSha: revision("c"),
      ...
      observation: { state: "draft", baseRevision: contract.repository.sourceRevision, headRevision: String(number).repeat(40),
        ...
        matchingOpenDrafts: 1, changedPaths: ["src/client.ts"], remoteTreeSha: revision("c"),
        evidenceRefs: [`github-observation-${number}`] },
    },
