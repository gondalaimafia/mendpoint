...
export type ExactDraftObservationEvidenceV1 = Readonly<{
  ...
  tenantId: string; cycleId: string; deliveryId: string; repositoryId: string;
  remoteRepositoryId: number; installationId: number; pullRequestNumber: number;
  baseBranch: string; branchName: string; baseRevision: string; headRevision: string;
  matchingOpenDrafts: number; changedPaths: readonly string[]; remoteTreeSha: string;
  verdict: "success" | "failure"; trigger: "checks_passed" | "ci_failure" | "review_feedback";
  requiredResults: readonly ExactDraftCheckResult[]; failures: readonly ExactDraftFailure[];
  reviewFeedback: ExactDraftReviewFeedback; reviewFeedbackDigest: string | null;
  evidenceRefs: readonly string[]; observedAt: string; observation: ExactDraftObservation;
}>;
...
async function observeDeliveryEvidence(octokit: Octokit, input: ExactDraftObservationInput, headRevision: string) {
  ...
  if (comparisonResponse.data.base_commit.sha !== input.expectedBaseSha || comparedCommits.length === 0 ||
      comparedCommits[comparedCommits.length - 1]?.sha !== input.expectedHeadSha) {
...
export async function observeExactDraftWithOctokit(
  ...
  return Object.freeze({
    ...
    reviewFeedback,
    ...(deliveryEvidence ? { repositoryId: deliveryEvidence.repositoryId,
      installationId: deliveryEvidence.installationId, matchingOpenDrafts: deliveryEvidence.matchingOpenDrafts,
      changedPaths: deliveryEvidence.changedPaths, remoteTreeSha: deliveryEvidence.remoteTreeSha } : {}),
