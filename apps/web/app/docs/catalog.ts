...
export const PRODUCT_DOCS: readonly ProductDoc[] = Object.freeze([
  ...
  page({
    ...
    interfaces: [api("GET /audit", "Read tenant scoped audit records."), api("GET /audit/export", "Export the governed tenant evidence set."), api("GET /audit-governance/legal-holds", "List active and released legal holds."), api("POST /audit-governance/legal-holds", "Place a legal hold over audit records."), api("POST /audit-governance/legal-holds/:id/release", "Release a legal hold."), api("GET /audit-governance/destinations", "List registered export destinations."), api("POST /audit-governance/destinations", "Register an export destination."), api("POST /audit-governance/destinations/:id/revoke", "Revoke an export destination."), api("GET /audit-governance/retention", "Evaluate retention disposition for tenant audit records."), api("POST /audit-governance/exports", "Create a redacted replay verifiable export manifest."), api("GET /audit-governance/exports/:id", "Read one export manifest and its bundle."), api("GET /audit-governance/exports/:id/replay", "Replay verify one stored export."), artifact("Audit chain", "Append only attributable transition records."), artifact("Compliance evidence package", "Engineering controls plus separately supplied legal and assessor evidence.")],
