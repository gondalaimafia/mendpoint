...
export const PRODUCT_DOCS: readonly ProductDoc[] = Object.freeze([
  page({
    ...
    limitations: ["Language and migration coverage is bounded and unsupported work can abstain.", "Review-comment feedback reentry is not yet implemented.", "Production depth depends on the connected repository and configured verifier."],
  ...
  page({
    ...
    capabilities: ["Exact-base branch creation and content-addressed Git commits", "File bytes and executable mode preservation", "Draft-only pull request creation", "Same-request replay, response-loss adoption, and drift rejection", "Required-check observation and bounded same-branch Fettler repair after fresh approval"],
    ...
    guardrails: ["Delivery is draft-only and cannot merge or deploy.", "Human approval is candidate-specific and expires on changed authority.", "The final remote side effect requires a current lease, pause state, expected head, and one-use intent."],
    limitations: ["GitHub review comments are observed only as limited state today; full requested-change feedback reentry is next work.", "Production availability depends on App permissions and exact tenant-repository installation binding.", "Cross-SCM feature parity is not complete."],
  ...
  page({
    ...
    interfaces: [api("POST /advanced-ai/training-jobs", "Run or reconcile an external training job when enabled."), api("POST /advanced-ai/post-trained-applications", "Register an exact completed adapter lifecycle."), api("POST /advanced-ai/post-trained-applications/:id/route", "Evaluate router eligibility without invoking a model."), artifact("Authenticated trainer receipt", "Job, request, artifact, evaluation, canary, and receipt MAC binding.")],
  ...
  page({
    ...
    interfaces: [command("npm run learning:export-corpus", "Export a governed learning corpus."), api("GET /learning/suppressed", "Inspect suppressed learning records."), artifact("Learning capture", "Outcome, consent, lineage, policy, and evidence references."), artifact("Sealed corpus", "Purpose-bound deterministic eligible dataset.")],
