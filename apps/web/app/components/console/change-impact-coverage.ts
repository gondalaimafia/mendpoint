...
export function changeImpactCoverageSummary(
  ...
  if (coverage.impact === "impact") {
    return {
      ...
      detail: `${coverage.findingCount} finding${coverage.findingCount === 1 ? "" : "s"} across ${coverage.prCount} staged PR${coverage.prCount === 1 ? "" : "s"}. Empty-findings wording does not apply.`,
      files: null,
      gaps: [],
    };
  }
