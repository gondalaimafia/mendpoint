...
describe("release dispatch drainer", () => {
  ...
  it("records unknown sink failures as retryable fixed-code failures and counts exhaustion", () => {
    const releaseStore = store();
    const appDb = db();
    ...
    expect(summary).toMatchObject({ claimed: 2, retried: 1, exhausted: 1, failed: 0 });
    const settled = listReleaseDispatches(releaseStore, "tenant-a");
    expect(settled.map((item) => item.lastFailureCode))
      .toEqual([RELEASE_DISPATCH_SINK_FAILURE_CODES.infrastructureUnavailable,
        RELEASE_DISPATCH_SINK_FAILURE_CODES.infrastructureUnavailable]);
