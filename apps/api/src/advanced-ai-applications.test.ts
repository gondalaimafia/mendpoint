...
describe("advanced AI applications API", () => {
  ...
  it("registers only after independent holdout evaluation and canary evidence", async () => {
    ...
    const checkpoint = await app.request("/advanced-ai/post-trained/adapters/adapter-1/proof-checkpoints", { method: "POST", headers: { "content-type": "application/json", "idempotency-key": "rollback-register" }, body: JSON.stringify({ inputDigest: `sha256:${"c".repeat(64)}`, task: task(), rollback: { expectedArtifactDigest: trained.adapterDigest, reason: "Canary regression", idempotencyKey: "rollback-register" } }) }); expect(checkpoint.status).toBe(201); expect(await checkpoint.json()).toMatchObject({ eligible: true, adapterId: "adapter-1", inputDigest: `sha256:${"c".repeat(64)}`, eventHash: expect.stringMatching(/^[a-f0-9]{64}$/) });
