...
export function createTransformerAttemptCoordinatorRoutes(options: Readonly<{
  ...
  app.post("/operations/:operation", async (c) => handled(c, async () => {
    ...
    const bound = { ...input, observedAt, ...(options.gateConfig === undefined ? {} : { gateConfig: options.gateConfig }) };
  ...
  app.post("/checkpoint-authority/:operation", async (c) => handled(c, async () => {
    ...
    const bound = { ...input, ...(!preserveBoundTime && operation !== "readHead" && operation !== "readBindingAuthority" ? { observedAt } : {}), ...(options.gateConfig === undefined ? {} : { gateConfig: options.gateConfig }) };
