...
**Source of truth:** monorepo `C:\Users\Talal\dev\mendpoint` (Warden / Mendpoint).  
...
| Run tests / verify loop | **3** | Warden `verifyCommand`; repair loop; CI comment helpers |
...
| Warden API-debug patches | **3.5** | trained failure modes, tool loop |
...
2. Single pipeline flag: `patch → test → PR` with fail → repair/Warden retry (budgeted).  
...
| Resolve rate (end-to-end unassisted) | **1–2** | Warden fixture tests; no published suite of 20–50 issues |
...
This is the **largest batch gap** relative to “working, benchmarked version of Warden.”
...
| PR copy / footers “human review” | **3–4** | branding / Warden footers (P0.2 in progress) |
...
| Patch generation | Public GitHub | Benchmark resolve rate | **3** | Demo/examples/Warden tests; resolve rate informal |
...
*“We have a working Warden loop on fixtures and a real GitHub path; pre-customer batch completion means finishing flagship feeds, internal issue-bench, exposure/Slack surfaces, then design partners.”*
...
- Completing **Batch-A + Batch-B** above is the honest definition of “first design-partner conversation against a working, benchmarked Warden.”  
