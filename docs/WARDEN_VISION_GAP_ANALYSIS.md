# Warden vision gap analysis

**Source vision:** Mendpoint “Introducing Warden” product narrative (Cognition/Devin-style launch structure).  
...
The intro sells **Warden** as a continuously watching, long-horizon integration engineer that:
...
| Autonomous bug find | Silent drift, repro, fix unprompted | 2 | Warden needs goal/error/verify; not background hunter |
| Self-correct on CI | Patch fails tests → iterate | 2–3 | Repair loop + Warden verify; limited CI integration |
...
**Overall readiness as design-partner narrative with caveats:** **plausible** if language is tightened to “scaffolded platform + Warden agent on partner repos” rather than continuous unprompted production teammate.

---

## 1. Product framing: who is “Warden”?

| Vision | Codebase reality | Gap |
|--------|------------------|-----|
| Warden is *the* product teammate | **Mendpoint** = monorepo/platform; **Warden** = `@mendpoint/agent` (`runWarden`) + UI `/agent` | Split identity: pipeline/PR path is not branded as Warden end-to-end |
| Same category as Devin (long-horizon) | Warden default **max ~20–24 steps**, heuristic + optional LLM tool suggest | Not thousands of decisions; no persistent multi-hour sessions |
| Watches *or* independently fixes | Pipeline = reactive on version/change; Warden = on-demand run | No unified “always on Warden” daemon per customer |

**Gap (P0 narrative):** Unify product language so **pipeline + repair + agent** are one **Warden** experience (or explicitly “Mendpoint platform / Warden agent” and don’t claim they are one loop).
...
| Reproduce bug | **2** | Warden runs `verifyCommand` if provided; does not invent repros |
| Fix without being asked | **1** | Warden requires **goal** (+ path); no unsolicited bug hunting job |

**Gap:** Runtime/contract shadow testing, anomaly detection, unsolicited issue → Warden sessions.
...
| Warden verify loop | **3** | Tool loop until verify passes |
| Full CI (GitHub Checks) close the loop | **2** | CI comment/check helpers; not full “fail → re-run Warden on PR” product |

**Gap:** PR-scoped CI failure → automatic Warden re-attempt with budget.
...
| API-spec diffing | Yes | change-intel OpenAPI (pipeline); **not inside Warden tool loop as first-class tool** |
...
**Gap:** Spec-diff tool inside Warden; true sandbox; VCS-native session UX.

### 2.9 Collaborate with engineers (report, why, feedback)

| Piece | Status | Evidence |
|-------|--------|----------|
| Report what/why/changed | **3** | PR body + Warden markdown report + audit |
...
Vision: SWE-bench-style **API regression** corpus; Warden resolves meaningful share **unassisted**; ahead of general agents; **publish technical report**.
...
| Unassisted (no file hints) | Harness has expected sites for scoring impact; Warden fixture is guided by goal text | 2 |
...
**Naming gap:** Intro uses **Warden** for the whole loop; code still says **pipeline / repair / agent**. Product should either rebrand the loop as Warden or rewrite intro to three products under Mendpoint.

---

## 7. Priority gap-closure program (recommended)

### P0 — Claim safety (before public “Introducing Warden”)

1. **Rewrite performance section** until `warden-bench` exists; ship internal numbers only.  
2. **Unify brand:** UI + PR footers + docs: “Opened by Warden” for pipeline + agent.  
...
| Silent drift | Contract test / response schema probe job → Warden session |
...
- CI fail → auto Warden re-run (budgeted)  
...
| “without being asked” | “when a change is detected or a Warden run is requested” |
...
| Feature adoption | 2 |
| Warden debug agent (API bugs) | 3.5 |
...
- **Do** position honestly: *Mendpoint is building Warden — a graph-leaned, human-reviewed integration teammate. Today: OpenAPI-driven impact → PR, plus on-demand Warden agent trained on API communication failures. Design partners next for continuous multi-repo watch.*  
- **Closest-to-true differentiators already in code:** hybrid **graph** impact, **never auto-merge** policy, multi-language fixtures, **Warden** failure-mode training, GitHub App path, adopt vs migrate modes.  
...
- `docs/API_BUG_AGENT.md` — Warden agent loop  
