...
**Supersedes:** Mendpoint Product and Platform Specification v3.0 as the development baseline


---

## Version 4.0 architectural refinement

Version 4.0 preserves the Fettler/ReGauge product boundaries and the representation-first architecture established in v3, while adding the persistent operating context required for long-running migration work and a pluggable structural-graph extraction boundary.

The following decisions now form the development baseline:
...
3. **Persistent Mission intelligence.** A Mission is not a thin execution record. It is the persistent operating context for a migration: graph version, repository snapshots, decisions, tasks, verification evidence, exceptions, artifacts, policy, cost, and history MUST survive across agent and human interactions.
4. **Organization Memory is separate from system truth.** Tenant-specific engineering conventions and preferences MAY persist across Missions, but MUST remain distinct from Change Graph facts, hard policy, migration recipes, and model weights.
5. **Inherited policy envelopes.** Every Mission task MUST inherit deterministic repository, tool, model, residency, risk, review, and learning-data boundaries. Prompts are not the primary security boundary.
6. **Shared human/agent task state.** Fettler, ReGauge, forward-deployed engineers, and customer reviewers SHOULD operate on one Mission task graph so work can move agent → human → agent without reconstructing context.
7. **Pluggable structural extraction.** Low-level code graph extraction is an interchangeable subsystem behind a Mendpoint-owned contract. External engines such as Graphify MAY be used when benchmarks show an advantage, but MUST NOT define Mendpoint's canonical ontology, persistence model, tenant semantics, or migration intelligence.
8. **Explicit epistemic state.** Graph edges, memory items, coverage, staleness, conflict, and provenance are first-class product data. `NO IMPACT FOUND` and `NO IMPACT FOUND WITH INSUFFICIENT COVERAGE` are different product outcomes.
9. **Selective intelligence ownership.** Mendpoint SHOULD own migration-specific intelligence where proprietary data, quality, latency, privacy/control, or economics justify it while retaining rented general reasoning where external models remain superior.
10. **Independent soft verification.** Deterministic verification remains authoritative. DeepSeek V4 Flash or another approved verifier MAY improve candidate selection and calibration but MUST remain a soft signal beneath tests, graph invariants, runtime evidence, policy, and human decisions.
11. **Proof-carrying migration.** A material migration SHOULD carry the evidence explaining why code is affected, what changed, what was verified, and what remains uncertain. Reviewability is part of the product, not a presentation layer added after generation.
12. **Consumer-first initial wedge.** The initial design-partner motion SHOULD prioritize engineering teams that depend heavily on third-party APIs, SDKs, and platforms. Provider-side campaign products remain a valid secondary/future motion but MUST NOT compromise the clarity of the initial buyer and workflow.

External projects and research may inform implementation choices, but their benchmark results are not Mendpoint product claims. Mendpoint MUST validate graph representation, structural extraction, persistent context, memory, and verification policies on its own synthetic and production-like migration benchmarks before relying on them for customer-facing performance or economic claims.
...
The previous product specification established the core thesis, the Fettler/ReGauge product split, graph-scoped reasoning, review-first execution, hybrid model orchestration, migration data as a compounding asset, and the land-and-expand product strategy. Version 4.0 preserves those decisions and further formalizes persistent Mission intelligence, Organization Memory, policy inheritance, human/agent task handoffs, pluggable structural extraction, representation epistemics, intelligence ownership, and independent-verification contracts required to build, evaluate, operate, and evolve the platform coherently.
...
## 0.4 Claude Code development contract

When this file is provided to Claude Code, it SHOULD be treated as the canonical product-development baseline.

Claude Code MUST:

1. read the relevant sections before changing product behavior or architecture;
2. inspect current code before assuming a specified capability is implemented;
3. treat this specification as the intended contract and the repository as the current implementation state;
4. preserve backward compatibility where historical identifiers or persisted schemas require it;
5. create or update an ADR when a substantive implementation choice departs from this baseline;
6. prefer extending existing subsystems over creating duplicate graph, Mission, memory, router, verification, or learning platforms;
7. add tests/evals for behavior it changes;
8. surface unresolved product decisions instead of silently inventing durable policy;
9. follow the repository's dual-agent operating protocol and request Codex peer review for material Claude-authored changes.

This section governs implementation workflow only; it does not change the product behavior defined elsewhere in this specification.
...
+ Mission Spaces, decisions, exceptions, artifacts, and persistent history
+ Organization Memory and tenant-specific engineering conventions
+ policy envelopes and inherited task boundaries
+ context compiler and task decomposition
+ model/router layer
+ deterministic tools and recipes
+ code execution and verification
+ human/agent task handoffs and review workflow
+ evaluation
+ learning flywheel
+ observability
```
...
A **Mission Space** is the durable operating environment for one migration or change-adoption effort. The persisted domain object MAY continue to be named `Mission` in code and APIs; the product contract is what matters.

A Mission Space MUST retain, directly or by stable reference:

- objective or trigger;
- tenant and repository scope;
- immutable repository snapshot references;
- provider/API/SDK version context where relevant;
- Change Graph version and task-specific graph projections;
- active and historical Mission decisions;
- task state and ownership;
- human/agent handoff history;
- verification history scoped to the code/provider state it verified;
- unresolved and resolved exceptions;
- artifacts such as impact reports, plans, patches, PRs, rollback plans, and graph diffs;
- policy envelope version;
- relevant Organization Memory snapshot or retrieval context;
- cost/MCU attribution;
- final outcomes and learning provenance.

A Mission MUST NOT rely on conversational transcript alone as its durable state.

### 6.5.1 Mission persistence principle

A later task SHOULD inherit what earlier work established without requiring a human to repeat it.

Examples:

- a ReGauge stage MUST know that an earlier stage established a migration ordering constraint;
- a Fettler repo task SHOULD inherit the provider-change interpretation already accepted for the campaign;
- a rejected approach SHOULD NOT be proposed again unless new evidence invalidates the original rejection;
- prior verification MAY be reused only when it remains valid for the relevant repository/provider snapshot.

## 6.6 Organization Memory

**Organization Memory** is tenant-specific persistent knowledge about how an engineering organization prefers or requires software to be changed.

Examples:

- all payment-provider traffic must pass through an internal gateway abstraction;
- database migrations require a separately approved stage;
- generated code is not edited directly;
- changes in a regulated service require a particular integration suite;
- PRs above a configured risk/size threshold require a staff reviewer.

Organization Memory MUST remain separate from:

- Change Graph facts describing what software is and how it relates;
- Mission decisions scoped to one migration;
- hard policy enforced deterministically;
- deterministic migration recipes;
- shared/global Mendpoint model weights.

Memory items MUST support provenance, scope, status, editability, disabling/deletion, and freshness semantics.

### 6.6.1 Memory lifecycle

A learned memory SHOULD use a governed lifecycle conceptually equivalent to:
...
with terminal or alternate states such as:
...
One reviewer edit MUST NOT automatically become durable Organization Memory.

### 6.6.2 Memory precedence

Default precedence SHOULD be:

```text
hard policy
> explicit current Mission decision
> confirmed Organization Memory
> user preference
> inferred/candidate memory
```

Inferred memory MUST NOT override security, tenancy, compliance, explicit task requirements, or current Mission decisions.

## 6.7 Policy Envelope

Every Mission MUST reference a versioned **Policy Envelope** that constrains all child tasks.

The envelope MAY include:

- tenant and repository scope;
- allowed branches/environments;
- forbidden edit zones;
- allowed tools;
- allowed model classes/providers;
- external-verifier eligibility;
- code/data export restrictions;
- residency;
- risk ceiling;
- auto-edit/autonomy limits;
- review requirements;
- deployment permissions;
- training-data eligibility;
- retention constraints.

Policy MUST be enforced deterministically where feasible. A prompt reminder is not an authorization control.

A long-running Mission MUST retain the policy version under which a decision was made. Policy upgrades during an active Mission MUST be explicit and auditable.

## 6.8 Shared Mission Task Engine

A **MissionTask** is the shared work primitive for agents and humans.
...
The task engine MUST support agent → human → agent continuation without reconstructing Mission state manually.

## 6.9 Mission decisions, exceptions, and artifacts

### MissionDecision

A durable decision with future consequence.

Examples:

- use a compatibility shim in stage 1;
- postpone schema deletion until stage 4;
- do not edit generated clients;
- require backward compatibility for two releases.

A decision SHOULD include scope, author, evidence, status, timestamp, and supersession.

### MissionException

A persistent blocker, constraint, or unresolved condition.

Examples:

- runtime relationship cannot yet be verified;
- unsupported legacy component;
- missing owner approval;
- migration stage blocked by another service.

An exception SHOULD include reason, impact, owner, blocking status, resolution path, and lifecycle state.

### MissionArtifact

A typed reference to material output such as:
...
Large artifacts SHOULD be referenced rather than duplicated inside Mission state.

## 6.10 Mission Context Compiler

Every substantial Fettler/ReGauge model or tool task SHOULD receive a bounded context assembled from structured Mission state rather than a full history dump.
...
Its objective is **minimum sufficient inherited context**.

The system SHOULD measure:

- relevant vs irrelevant inherited context;
- context tokens;
- repeated retrieval avoided;
...
## 7.4 Layer D — Mission Spaces, context compilation, orchestration, and routing
...
Memory MUST be tenant-scoped and MUST NOT influence another tenant.
...
A normalized audit/timeline event MAY record meaningful state transitions such as task assignment, decision creation, verification completion, graph-version advance, memory confirmation, exception resolution, or Mission completion.

Mendpoint MAY implement these events without adopting full event sourcing.
...
If graph coverage is insufficient, Fettler SHOULD be able to perform bounded targeted retrieval/exploration and feed newly validated stable relationships back into graph improvement.

### FET-019 — Campaign Mission persistence

A provider-change campaign spanning multiple repositories MUST preserve the accepted provider-change interpretation, shared policy, common migration decisions, per-repository task state, and prior verification results across the Mission.

### FET-020 — Proof-carrying migration package

A material Fettler PR SHOULD expose: what changed upstream, why the affected path is in scope, the evidence path through the Change Graph, the proposed edit, verification evidence, graph/coverage limitations, and unresolved uncertainty.

### FET-021 — Persistent decision avoidance

Fettler SHOULD NOT repeatedly propose an approach that the same Mission has already rejected unless new evidence or a superseding decision justifies revisiting it.
...
ReGauge planning SHOULD consume an explicit MissionGraphProjection and expose the dependency/evidence paths that justify stage ordering and migration boundaries.

### REG-019 — Long-running Mission continuity

Multi-stage ReGauge campaigns MUST preserve decisions, exceptions, verification state, active constraints, graph-version transitions, and human resolutions across stages.

### REG-020 — Human-agent resume

When ReGauge escalates a graph gap, architectural ambiguity, policy exception, or high-risk decision to a human, the human's resolution MUST update persistent Mission state so ReGauge can resume without reconstructing the issue from scratch.

### REG-021 — Organization convention application

Confirmed Organization Memory MAY influence ReGauge plans and edits when it is in scope, current, and subordinate to hard policy/current Mission decisions. The system SHOULD expose which memory influenced a material recommendation.
...
## 11.22 Structural extraction layer

Mendpoint SHOULD separate **structural code extraction** from **semantic migration intelligence**.

The structural layer is responsible for low-level relationships such as:
...
The semantic Change Graph remains responsible for higher-order relationships such as:
...
Structural extraction MUST sit behind a Mendpoint-owned interface so an extractor can be replaced without leaking its types through product APIs.

A logical contract SHOULD expose:
...
## 11.23 External structural extractor evaluation

Mendpoint MAY use external/open-source extraction engines when they improve quality or engineering efficiency.

**Graphify** is a candidate structural extractor because it provides local tree-sitter based code parsing, cross-file structural relationships, explicit extracted/inferred/ambiguous edge states, graph diffing, incremental/watch capabilities, community analysis, and path/query primitives.

Graphify MUST NOT be treated as Mendpoint's canonical Change Graph, ontology, tenant model, storage architecture, or migration reasoning layer.

Before production adoption, Mendpoint MUST benchmark the candidate extractor against the current path using known-ground-truth synthetic repositories and representative repository scale.

The decision MUST be one of:

```text
ADOPT
ADOPT_FOR_SELECTED_LANGUAGES
ADOPT_AS_FALLBACK
INTERNAL_TOOL_ONLY
DO_NOT_ADOPT
```

and SHOULD be recorded in an ADR or equivalent engineering decision.

## 11.24 Structural-to-semantic promotion

External structural edges MUST pass through Mendpoint normalization, entity resolution, provenance mapping, and semantic promotion.

Example:

```text
structural:
createPayment CALLS stripe.PaymentIntent.create

provider resolution:
stripe.PaymentIntent.create IMPLEMENTS_ENDPOINT POST /v1/payment_intents

semantic promotion:
createPayment USES_ENDPOINT POST /v1/payment_intents
```

A promoted semantic edge SHOULD retain lineage to the structural/provider evidence that supports it.

## 11.25 Extractor epistemic mapping

If an extractor emits confidence/status such as:
...
Mendpoint SHOULD preserve the original value in provenance and map it into Mendpoint's richer epistemic state.

External confidence labels MUST NOT silently become calibrated Mendpoint probabilities.

## 11.26 Architecture discovery signals

Graph community detection, centrality, fan-in/fan-out, and similar structural metrics MAY be used as **features** for ReGauge architecture reconstruction, blast-radius analysis, or router risk.

They MUST NOT be treated as architectural truth or standalone risk scores.

Candidate subsystem boundaries SHOULD be corroborated using repository structure, runtime evidence, deployment metadata, ownership, data-store usage, or human evidence.
...
- Mission stage and history;
- prior failed/rejected approaches;
- active Mission decisions and exceptions;
- relevant Organization Memory;
- Policy Envelope requirements;
- risk;
- blast radius;
...
## 17.4.4 Mission and Organization Memory learning

Validated outcomes MAY improve persistent Mission or Organization intelligence without entering model weights.

The lesson destination classifier SHOULD distinguish:
...
- a human resolves a one-off migration sequencing question → `MISSION_DECISION`;
- a repeated tenant-wide convention is confirmed → `ORGANIZATION_MEMORY`;
- an internal wrapper relationship was missing → `GRAPH`;
- a repeatable exact transformation is verified → `RECIPE`;
- the correct graph/context was supplied but the model repeatedly violates it → candidate `MODEL_WEIGHT`/harness intervention.

Organization-specific preferences SHOULD remain tenant memory unless a separate, governed private-adaptation decision justifies training.
...
## 19.7.1 Mission and Memory isolation

Mission Space state and Organization Memory are tenant-private.

The platform MUST prove that:

- Tenant A cannot read Tenant B Mission state;
- Tenant A memory cannot be retrieved into Tenant B context;
- disabled/deleted memory is not compiled into future task context;
- policy scope cannot be widened by model output;
- external verifier/model calls are blocked when the Mission Policy Envelope forbids external processing.

## 19.7.2 Memory injection safety

Repository contents, issue text, generated artifacts, and arbitrary model output MUST NOT directly create confirmed Organization Memory.

Memory candidates MUST originate through approved learning paths and MUST preserve provenance.

Untrusted repository instructions are data, not authority.
...
## 20.7.1 Mission concurrency and resumability

Mission updates MUST avoid silent last-write-wins loss of decisions, exceptions, or task transitions.

The platform SHOULD use scoped optimistic concurrency or equivalent version controls so:

- human decisions made while an agent is working are not silently overwritten;
- independent task completions can be merged safely;
- duplicate callbacks do not duplicate state or cost;
- a blocked task can resume from persisted state after process/session failure.
...
## 22.5.1 Mission and memory metrics
...
exception count
repeated-error rate
repeated-instruction rate
compiled context tokens
Organization Memory items used
memory candidates / confirmed / rejected / stale
memory-caused errors
cost / MCU by Mission and task
```

Persistent context SHOULD be judged by reduced repeated work and improved migration consistency, not by the volume of stored memory.
...
## 23.1.1 Mission-first UX

The Mission SHOULD be the primary persistent work surface for multi-step work.

A Mission experience MAY expose:
...
ReGauge campaigns in particular SHOULD remain navigable across stages without forcing the user to reconstruct prior decisions.

Organization Intelligence SHOULD be a separate tenant-level surface for confirmed conventions and memory, including provenance, scope, status, edit/disable/delete controls, and training eligibility where relevant.
...
## 24.5 Structural extraction engines

Mendpoint MUST expose a product-owned structural extraction contract.

The platform MAY support:

- internal extractors;
- language-specific static-analysis tools;
- Graphify or similar open-source structural extraction;
- fallback or multi-extractor comparison for selected languages.

External extractor adoption MUST be version-pinned, benchmarked, security-reviewed, and feature-flagged where operationally feasible.
...
The initial primary design-partner buyer SHOULD be an engineering organization that depends materially on third-party APIs, SDKs, infrastructure, or developer platforms.

Provider-side use cases are strategically valid but SHOULD remain a secondary motion until customer-side design-partner evidence justifies broadening the primary narrative.

A preferred early engagement is:

```text
one real provider/API/SDK change
+ one to three repositories
+ explicit before/after success criteria
```

The goal is to prove impact mapping, migration correctness, verification, and reviewer acceptance before optimizing public packaging or pricing tiers.
...
- persistent Mission Space semantics;
- Mission decisions, exceptions, artifacts, and human/agent task handoff;
- Organization Memory contract and candidate lifecycle;
- versioned Policy Envelope;
...
- pluggable structural-extractor contract;
- Graphify/current-extractor side-by-side evaluation or explicit deferral;
- raw-vs-graph representation benchmark;
- stateless-vs-persistent-context benchmark;
...
- stage verification;
- campaign dashboard;
- persistent ReGauge Mission continuity across stages;
- human escalation and agent resume;
- Organization Memory application with provenance and precedence.
...
- persistent Mission intelligence and Organization Memory evaluated against a stateless control;
- structural extractor decision recorded with evidence.
...
## 28.1.0 Mission and persistent-context acceptance criteria

A production-grade Mission system MUST demonstrate:

- stable repository and graph snapshot binding;
- decisions persist and can be superseded;
- exceptions persist and can be resolved;
- verification history is scoped to the state it verified;
- tasks can hand off agent → human → agent without losing context;
- the Mission Context Compiler includes relevant active state and excludes unrelated/stale history;
- Policy Envelope restrictions are inherited and enforced;
- Organization Memory is tenant-scoped, inspectable, disableable, and subordinate to hard policy/current Mission decisions;
- duplicate callbacks/task completions are idempotent;
- concurrent updates do not silently erase state;
- Mission cost/MCU usage can be attributed.
...
## 31.6.3 Preserve persistent experience structurally

Do not use chat/session transcripts as the primary persistence layer for long-running migration context.

Persist decisions, exceptions, verification, tasks, artifacts, policy, graph versions, and confirmed Organization Memory as typed domain state.

## 31.6.4 Separate structural extraction from migration semantics

Low-level source parsing and structural graph extraction MAY be commoditized or replaced.

Mendpoint's defensibility SHOULD live above that layer in semantic resolution, temporal Change Graphs, evidence, coverage, migration constraints, Mission intelligence, verified outcomes, and learning.
...
## 33.12 Structural extractor adoption

Should Graphify become:

```text
primary structural extractor
selected-language extractor
fallback
internal engineering tool
or not adopted
```

Decision MUST be benchmark- and security-driven.

## 33.13 Organization Memory activation

What evidence threshold, review workflow, and tenant controls are required before an inferred memory candidate becomes active?

## 33.14 Policy upgrades during active Missions

When organization policy changes, which active Missions retain their original Policy Envelope and which must explicitly migrate to the new version?

## 33.15 User-level memory

When, if ever, should reviewer-specific presentation/workflow preferences become a separate User Memory layer? This is lower priority than Organization Memory.


## 33.16 Provider-side product
...
The persistent operating context for one migration, including graph/snapshot state, tasks, decisions, verification, exceptions, artifacts, policy, cost, and history.

### Organization Memory

Tenant-specific, inspectable persistent knowledge about engineering conventions and preferences. It is distinct from Change Graph facts, hard policy, recipes, and model weights.

### Policy Envelope

The versioned deterministic constraints inherited by Mission tasks, including repository/tool/model scope, residency, review, deployment, and learning-data permissions.

### Mission Context Compiler

The component that assembles the minimum sufficient task context from Mission state, graph projection, Organization Memory, policy, verification history, and exceptions.

### StructuralGraphExtractor

A Mendpoint-owned interface for low-level code graph extraction. Implementations may include internal parsers or external engines such as Graphify.

### Proof-carrying migration

A migration package that exposes not only the proposed code change but the evidence path, verification state, coverage limitations, and unresolved uncertainty required for review.
...
## 35.3 Persistent Mission flow

```text
Mission created
→ bind repository snapshots + Change Graph version + Policy Envelope
→ compile first task context
→ agent executes
→ verification/evidence recorded
→ decision / exception / artifact persisted
→ task completes or hands off to human
→ human resolution updates Mission state
→ next agent task inherits active decisions + relevant Org Memory + graph projection
→ accepted code advances repository snapshot + graph version
→ Mission continues until complete
→ outcome feeds learning
```

The Mission MUST remain resumable across model sessions, process restarts, and human/agent handoffs.
