...
- `MENDPOINT_REGAUGE_LIVE_EVAL_TENANT=<tenant id>`

Without `MENDPOINT_EVAL_LIVE_REGAUGE=1` the runner throws
...
- `MENDPOINT_REGAUGE_ADAPTIVE_MODEL_SOURCE_TENANTS=<tenant id>` (must include the eval tenant)
- `MENDPOINT_REGAUGE_ADAPTIVE_MODEL_PROVIDER=<provider id>`
- `MENDPOINT_REGAUGE_ADAPTIVE_MODEL_DEPLOYMENT=<deployment id>`
- `MENDPOINT_REGAUGE_ADAPTIVE_EXTERNAL_PROCESSING_APPROVED=1`
- `MENDPOINT_REGAUGE_ADAPTIVE_EXECUTION_REGION=<region>`
- `MENDPOINT_REGAUGE_ADAPTIVE_MAX_DATA_CLASSIFICATION=<classification>`
...
- `MENDPOINT_REGAUGE_LIVE_EVAL_MAX_USD` (default 25). A present but invalid
...
(both default 1, override with `MENDPOINT_REGAUGE_LIVE_MIN_PASS_RATE` and
...
- `transformer-live-eval.test.ts` exercises the runner with an injected mock
  ...
  call.
- The production gate `MENDPOINT_REGAUGE_GATE` default stays denied and the
