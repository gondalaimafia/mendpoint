...
**Canonical doctrine.** Absorbed from [Graph Engineering vs Loop Engineering](https://www.aibuilderclub.com/blog/graph-engineering-vs-loop-engineering) (AI Builder Club) and applied to Fettler / Mendpoint.
...
| Mendpoint examples | Fettler tool loop; repair session loop | Change → impact fan-out → generate → verify → review **org chart** |
...
**We default to graphs for product work.** Single-agent loops are valid *nodes* (Fettler, repair). They are not the whole system.
...
1. **Prompt** — per-node system/user instructions (Fettler playbook, LLM confirm)  
...
## Go-to topology (Fettler product graph)
...
| `verify` | **Fettler / repair loop** | goal + verify command + repo |
...
|--------------------|----------------------|
| One specialty (e.g. Fettler fix given goal) | Distinct specialties (intel vs scan vs patch vs review) |
