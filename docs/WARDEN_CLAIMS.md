# Gauge — claim-safe public language

**Product:** **Gauge** (Mendpoint’s API integration teammate)  
...
| **Gauge on-demand tool loop** | Goal-driven API debug agent (`@mendpoint/agent`, `/agent`) with bounded multi-step tools |
...
| **Unprompted silent-drift hunting** | Gauge needs a goal (and path); not a background unsolicited bug hunter |
...
| “without being asked” | when change detected or Gauge run requested |
...
**Gauge** is Mendpoint’s API integration teammate: **graph engineering** (specialized verify-backed nodes, not one mega-agent), reviewable migration PRs, and an on-demand API debug loop-node. Continuous multi-repo watch and public benchmarks are on the design-partner track. Never auto-merges by default.
...
- Consumer registry + contract gates + Gauge debug loop  
