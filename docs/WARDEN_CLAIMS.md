# Fettler — claim-safe public language

**Product:** **Fettler** (Mendpoint’s API integration teammate)  
...
| **Fettler on-demand tool loop** | Goal-driven API debug agent (`@mendpoint/agent`, `/agent`) with bounded multi-step tools |
...
| **Regauge scaffold** | BSG + DAG campaign types (`@mendpoint/transformer`) — not full migration engine yet |
...
| **Unprompted silent-drift hunting** | Fettler needs a goal (and path); not a background unsolicited bug hunter |
...
| “without being asked” | when change detected or Fettler run requested |
...
**Fettler** is Mendpoint’s API integration teammate: **graph engineering** (specialized verify-backed nodes, not one mega-agent), reviewable migration PRs, and an on-demand API debug loop-node. Continuous multi-repo watch and public benchmarks are on the design-partner track. Never auto-merges by default.
...
- Consumer registry + contract gates + Fettler debug loop  
- Production probes (`/ready`, `/live`), rate limits, auth required in `NODE_ENV=production`  

**Do not say “GA” for:** Regauge full migration product, Firecracker isolation, GNN training, unprompted multi-repo hunting, or public benchmark leadership.
