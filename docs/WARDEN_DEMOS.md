...
| Fettler agent fixture demo | `npm run agent:demo` | ✅ |
| Fettler unit tests | `npm run agent:test` | ✅ |
| **warden-bench (internal)** | `npm run eval:warden` | ✅ |
| Fettler UI | `npm run dev:web` → `/agent` (+ `dev:api`) | ✅ |
...
| [`DESIGN_PARTNER_PATH.md`](./DESIGN_PARTNER_PATH.md) | Install → seed → poll → pipeline → PR → Fettler |
