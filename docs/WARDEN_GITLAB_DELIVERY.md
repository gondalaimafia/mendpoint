# Gauge GitLab delivery (Gap-closure G1a)

Gauge delivers an approved candidate as an exact draft: a source branch, one
...
GitHub draft pull request. GitHub stays the default and is unchanged when GitLab
is not selected.
...
message and date, and the review evidence title and body. That intent is
delivered as either a GitHub draft pull request or a GitLab draft merge request.
The review evidence body, branch name, and file set are identical across
providers; only the destination changes.
...
2. `commitFiles` commits the exact sealed files onto that branch and returns the
   created commit SHA.
...
reuses `GitLabDelivery` rather than rebuilding delivery, so the Gauge delivery
...
`gitlab_exact_draft_not_draft` and Gauge records a delivery failure instead of
reporting success. Nothing is merged and no branch is force-updated. Human
review and manual merge stay in force, the same as the GitHub path. Gauge's
...
create-commit response), so Gauge's evidence check accepts it exactly as it
...
Gauge construction site routes through the same `transformerAdaptiveScmDelivery`
...
| --- | --- | --- |
| `SCM_PROVIDER` | `gitlab` routes Gauge approved-candidate delivery to GitLab; unset or `github` keeps GitHub. | `github` |
...
- **Not a change to the GitHub path.** When GitLab is not selected, Gauge's
  GitHub delivery, its App and token enforcement, and the review-first,
  no-auto-merge model are unchanged.

## Summary for marketing

Gauge can deliver an approved candidate as a **token-authenticated draft merge
...
merge-request delivery via access token," not as a hosted GitLab App install.
