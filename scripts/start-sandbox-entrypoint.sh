...
iptables --wait -A OUTPUT -j REJECT --reject-with icmp-admin-prohibited
iptables --wait -P OUTPUT DROP
ip6tables --wait -F OUTPUT
ip6tables --wait -A OUTPUT -o lo -j ACCEPT
ip6tables --wait -A OUTPUT -j REJECT --reject-with icmp6-adm-prohibited
ip6tables --wait -P OUTPUT DROP

iptables --wait -C OUTPUT -o lo -j ACCEPT
iptables --wait -C OUTPUT -j REJECT --reject-with icmp-admin-prohibited
ip6tables --wait -C OUTPUT -o lo -j ACCEPT
ip6tables --wait -C OUTPUT -j REJECT --reject-with icmp6-adm-prohibited
