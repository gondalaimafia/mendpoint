...
function stagedSuccessorFixture() {
  ...
  // The new template bytes are base-pinned as an inert reviewed template (landed
  // by a prior runtime rotation); the executable workflow still carries the
  // previously staged digest until this re-stage overwrites it.
  input.basePolicy.protectedFiles[stagedSuccessor.templatePath] = replacementSuccessor.workflowSha256;
...
describe("production closure authority rotation", () => {
  ...
  it("rejects a re-stage whose new template bytes were never landed as a base-pinned template", () => {
    const { input, stagedSuccessor } = stagedSuccessorFixture();
    // Revert the base template pin to the OLD digest: the new bytes were never
    // landed on main as an inert reviewed template. #504 accepted this because it
    // base-pinned the OLD digest; the two-step requires the NEW template digest
    // to be base-pinned first.
  ...
  it("bounds the re-stage activation deadline against the original stage, not the current hop", () => {
    const { input, replacementSuccessor } = stagedSuccessorFixture();
    // The original stage (priorStage) was issued 2026-08-25T11:00Z. Move THIS
    // re-stage hop and the deadline forward so the deadline sits within seven
    // days of this hop but more than seven days from the original stage. #504
    // bounded each hop to its own issuedAt, so repeated re-staging extended the
    // window indefinitely; the canonical window is absolute.
    const receipt = input.proposedLedger.rotations.at(-1)!;
    receipt.issuedAt = "2026-08-30T11:00:00.000Z";
    receipt.expiresAt = "2026-08-31T11:00:00.000Z";
    input.observedAt = "2026-08-30T12:00:00.000Z";
    const late = { ...replacementSuccessor, activationDeadline: "2026-09-05T11:00:00.000Z" };
    input.proposedPolicy.successor = {
      phase: "staged",
      stagedByRotationId: "rotation-20260825-restage",
      activatedByRotationId: null,
  ...
  it("fails closed when the origin stage issue time cannot be parsed", () => {
    const { input } = stagedSuccessorFixture();
    // Corrupt the origin stage receipt's issue time in the immutable ledger.
    // Its activationDeadline is otherwise within the window, so the ONLY thing
    // that can reject it is the explicit finite-origin precondition; without it
    // deadline - NaN > MAX evaluates false and the absolute bound vanishes.
    input.baseLedger.rotations[0] = {
      ...input.baseLedger.rotations[0],
