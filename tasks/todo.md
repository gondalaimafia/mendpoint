...
## 2026-08-30 Retire obsolete legacy CI deployment

- [x] Trace every CI deployment job from trigger and protected gates through credential, Fly config, target app, and live verification.
- [x] Remove only the obsolete `deploy` job that targeted the stopped `mendpoint-talal` app.
- [x] Preserve the customer Fettler production deployment behavior and its dedicated credential boundary.
- [x] Add a workflow contract test requiring exactly one CI deployment target and prohibiting the legacy app.
- [x] Run focused workflow tests, configuration completeness, action pin and YAML checks, affected typecheck, and diff integrity.
- [x] Commit the isolated repair without pushing.

### Review

- The generic CI workflow no longer contains a `deploy` job or any `mendpoint-talal` target. Its only deploy-capable job is `deploy-customer-production`; ReGauge remains owned by its separate protected workflow.
- The customer job's parsed behavior is unchanged: the protected gate dependencies, current-main guard, customer-only Fly token, explicit customer config and app target, bounded machine recovery, exact revision assertion, and liveness-only deployment decision remain intact.
- Verification passes: 3 of 3 focused CI workflow tests, 16 of 16 configuration completeness tests plus the static gate, 6 of 6 action-pin tests plus the static gate, direct YAML parse, isolated strict typecheck of the changed test, parsed before-and-after customer job equivalence, and diff integrity.
- No push, merge, deployment, secret change, branch-protection change, or retirement of the stopped Fly app was performed. Fresh exact-head review and current-base protected CI remain required.
