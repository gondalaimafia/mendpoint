...
- NOT CONTINUOUS, deliberately: no workflow runs `recovery:proof`, so the RPO check does not fire on a schedule and cannot catch backups that silently stop. Wiring it into `customer-backup.yml` was considered and rejected. That workflow runs on the customer machine through `flyctl ssh console`, where the proof would contend for the same mutation fence as the backup it is meant to verify and would need a full restore plus a rollback copy of production data on the production host; its `backup` job is also gated on `vars.MENDPOINT_CUSTOMER_PROFILE_ACTIVE`, which is not set, so a proof step added there would never execute. Adding a step that structurally cannot run would make the gap look closed while leaving it open. The proof stays operator-run, and continuous RPO monitoring remains owed work under `ME-ENT-007`.
