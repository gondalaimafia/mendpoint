...
- Review repair: rollback safety no longer accepts operator-supplied activity counts. It compares the complete restored database evidence, authenticates the current cutover fence, and binds the signed proof to the transfer ID and unique fence marker so a proof cannot be replayed across cutovers.
