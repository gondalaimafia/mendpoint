...
## 2026-08-30 Production deploy local-build repair

- [x] Decode the complete failed deployment chain after the Depot bypass merged.
- [x] Replace the legacy remote-builder dependency with the GitHub runner's proven local Docker builder while retaining the app-scoped deploy credential.
- [x] Update workflow contract tests to distinguish local builds from both Depot and the legacy remote builder.
- [x] Run focused workflow tests, scripts typecheck, and diff integrity. Independent exact-head review remains pending.
- [ ] Merge through all six protected checks and verify the exact main revision on production.

### Scope and rollback

- The failed run proved app read authority but the legacy remote builder rejected the app-scoped token before reading the Dockerfile. This repair changes only the image build location; app scope, one-machine topology, exact revision stamp, machine restart guard, and live revision verification remain unchanged.
- Rollback reverts this isolated workflow change. No credential is widened and no readiness claim is promoted.

### Review

- Run `33347789001` proved all code, container, evaluation, release, and browser gates green, then failed before reading the Dockerfile because `--depot=false` selected Fly's legacy remote builder, whose authority is distinct from app deployment authority.
- The repair uses `--local-only`, the already available GitHub runner Docker daemon, and retains the app-scoped token, exact revision stamp, one-machine bound, machine-state recovery, and post-deploy revision and liveness proof.
- The focused workflow contract test passes, scripts TypeScript passes, and diff integrity is clean. Production proof remains pending normal protected merge.
