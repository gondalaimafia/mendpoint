...
The second exact-head review of `230d9a2c` found 11 P1 evidence and authority defects. The current uncommitted remediation requires Ed25519-signed, issuer-key-revision-time-bound production and provider governance authority tokens with runtime opacity. Evaluation rows bind to a signature-verified authority and validated production receipt, including tenant, repository, commit, production revision, snapshot, fixture digest, graph, policy, model, router, recipe, consent, authorization, sandbox, execution, and budget. Every metric and governance enum or boolean is exact-type validated. Mutation execution accepts only an admitted fixture token, rejects hidden Git index flags, validates pristine working bytes against the index, and removes any patch that creates an escaping symlink before returning failure. Reports withhold holdout answers, label 30 candidates assigned but unsealed, and now distinguish 55 native case bindings from 95 synthetic substrate bindings. Requirement rows report 15 planned, 86 unplanned, zero verified, and 101 unverified; planned oracle IDs are not test receipts. Generated reports remain explicit: zero retained source snapshots, zero admitted repositories, zero admitted fixtures, zero completed runs, and no customer readiness.

Current focused verification passes 60 of 60 across nine production learning files; the eval TypeScript project and `git diff --check` pass. The generated report set records 150 cases, 750 required arms, and zero executed arms. A fresh independent exact-head rereview and protected CI are required after the remediation commit. Existing wider-suite and external production evidence boundaries remain unchanged and are not promoted.
