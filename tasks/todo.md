...
## 2026-09-02 Test clock injection sweep (wall-clock principals vs hardcoded timestamps)

- [x] Inject each suite's existing fixed clock into the clock-less `createAuthMiddleware(db)` call so the api_key trust principal's `created_at` is stamped on the suite clock, not the real test-run wall clock.
- [x] Pin the wall clock in three adjacent suites whose control-plane rows are stamped by seam-less `nowIso()`/`new Date()` reads (`recordAudit`, `materializeConnectedRepository`, the regauge verifier-consent path).
- [x] Verify each edited file alone, prove the four previously-bombing suites pass under a +3 year clock shift, and confirm no product code and no assertion changed.

### Root cause

- `createAuthMiddleware(db, options)` materializes trust principals at `observedAt = (options.now ?? (() => new Date()))()` and writes `createdAt: observedAt.toISOString()` (apps/api/src/auth.ts:459, :646, :661). A test that builds its app with `createAuthMiddleware(db)` and no `now` stamps the api_key principal's `created_at` at the real test-run time while the rest of the suite runs on a hardcoded clock. Any production check of the form `principal.created_at <= <hardcoded timestamp>` then flips from pass to fail once the real clock passes that timestamp. The proven live instance is billing-economics: `packages/db/src/invoice-export.ts` requires `occurredAt >= actor.created_at`, and the three invoice-export tests return 403 instead of 201 from 2026-09-02T12:00:00Z onward.
- Three adjacent suites carry the same class of bomb through a different seam. audit-governance stamps its own control-plane audit events through `recordAudit`, which calls `nowIso()` (packages/db/src/index.ts:3723), and asserts a fixed 2035 retention disposition; repository-connections stamps snapshot `createdAt`/`expiresAt` with `nowIso()` (apps/api/src/repository-connections.ts:420) and purges at a fixed `at`; regauge reads the real clock for the verifier-consent check, which is only active inside the env-pinned window (effective 2026-08-24, expires 2026-11-20), so from 2026-11-20 the second bootstrap fails `regauge_verifier_consent_inactive`.

### Scope and rollback

- Test-only. No product (non-test) code changed. The billing-economics.test.ts injection landed on `main` through #587 (24590c4d) while this sweep ran and is no longer part of this branch's diff; this branch proves it under a shifted clock and carries the rest. Files: apps/api/src/change-detail-impact.test.ts, change-sources.test.ts, dashboard-routes.test.ts, design-partner-applications.test.ts, diagnostics-routes.test.ts, graphql-schema-ingestion.test.ts, outcome-metrics-routes.test.ts (group 1: inject the suite clock into `createAuthMiddleware`); audit-governance-routes.test.ts (fake Date pinned to the suite instant), repository-connections.test.ts (fake timers on the one materialize-and-purge test), regauge-production-bootstrap-runtime.test.ts (process Date pinned inside the verifier-consent window with `vi.useFakeTimers({ toFake: ["Date"], shouldAdvanceTime: true })`, because recipe-evidence freshness reads `new Date()` with no seam); and this task record.
- No hardcoded date was moved into the future, no assertion was loosened or deleted. The regauge clock is anchored inside the verifier-consent window (after effective_at 2026-08-24 and before expiry 2026-11-20), not after it. Rollback is a single revert of this branch; every change is inert on production behavior.

### Review

- Rebased onto `origin/main` 24590c4d (#587 merged mid-sweep carrying the byte-identical billing injection, so that file dropped out of this diff) and again onto 4bf8e1c0 (#534). Ten test files and this record changed; no product code. Independent Opus review on d79b3ca8: SAFE TO MERGE, all three pinned clocks proven load-bearing by mutation; its one should-fix (the regauge `shouldAdvanceTime` flag was not load-bearing) is applied, and the rebased tree re-passed the same gates, with the git-spawning suites that timed out under three concurrent vitest jobs re-run alone and green at the real clock, +30 days and +1095 days.
- Full `apps/api` suite in the fix worktree: 74 files / 703 tests pass at the real clock, at +30 days, and at +1095 days under a `NODE_OPTIONS --require` preload that shifts `new Date()`/`Date.now()`; all three runs exit 0. `billing-economics.test.ts` alone at a clock of 2026-09-02T12:01Z: 11/11.
- Pre-fix control on an unfixed `origin/main` worktree under the same +1095-day shift: billing-economics 3 failed (403 vs 201), regauge-production-bootstrap-runtime 1 failed (`regauge_verifier_consent_inactive`), repository-connections 1 failed (purge 0/0 vs 1/1), audit-governance-routes 1 failed (retention disposition). Each threshold was pinned by running the file alone one minute before and after it: 2026-09-02T12:00:00Z, 2026-11-20T23:59:59Z, 2027-07-18T00:00:00Z, 2028-08-31T13:00:00Z. `apps/web` is 275/275 and `apps/worker`/`packages/*` show no shift-only failures once their timeouts are re-run alone with a 120 s budget.
- `tsc -p apps/api/tsconfig.json --noEmit` exit 0; `npm run eol:check` exit 0; `npm run third-state:check` exit 0; `git diff --check` clean; 0 CR bytes in every changed file.
- Not verified here: CI on Linux runners; the codex peer review requested on the pull request.
## 2026-09-02 Process rules from #488 without the lane change

- [x] Land the uncontroversial process rules from Cursor's draft #488 against current `main`, phrased for the existing review lanes; exclude the review-lane switchover per the owner's 2026-09-02 split decision (keep "Claude-authored material PRs require Codex review"; introduce no Claude-reviews-Claude).
- [x] Reviewing-run identity: every agent review comment must name the reviewing run, because author and reviewer can post from one GitHub account and "attributable" alone cannot rule out self-review (`docs/agents/OPERATING_PROTOCOL.md` §11; reviewer-mode bullet in `CLAUDE.md` and `AGENTS.md`).
- [x] Red closure contexts are a merge gate: agents escalate, never admin-override; the contexts are named by their branch-protection source of truth, not a hardcoded string (`docs/agents/OPERATING_PROTOCOL.md` §14; item 10 in `CLAUDE.md` and `AGENTS.md`).
- [x] Stop-the-line: `CHANGES REQUIRED` / `NEEDS FIXES` halts merge work on a PR until the exact fixed head is re-reviewed (`docs/agents/OPERATING_PROTOCOL.md` §11; item 10 in `CLAUDE.md` and `AGENTS.md`).
- [x] `release-owner:<actor>` label rule (new; absent from #488 and `main`): exactly one label per PR; the closure authority skips the owner's own actor, so a `release-owner:claude` PR empties the trusted set and Claude-authored PRs are labelled for the Codex lane (`docs/agents/OPERATING_PROTOCOL.md` §14, citing `scripts/production-closure-github-authority.ts`).
- [x] Fix the dead job name `deploy` to `deploy-customer-production` where the docs name the production deploy job (`docs/agents/OPERATING_PROTOCOL.md` §10.1; verified against `.github/workflows/ci.yml`).
- [x] Replace hardcoded CI-check phrasing with "all required status checks per branch protection are green" (`docs/agents/DEFINITION_OF_DONE.md`, `.github/pull_request_template.md`, `docs/agents/OPERATING_PROTOCOL.md` §14).
- [x] Run `npm run eol:check`, `npm run docs:check`, `git diff --cached --check`, and a 0-CR byte count; open the PR (not draft), labelled `release-owner:codex`.

### Scope and rollback

- Docs only. Owned files: `AGENTS.md`, `CLAUDE.md`, `docs/agents/OPERATING_PROTOCOL.md`, `docs/agents/DEFINITION_OF_DONE.md`, `.github/pull_request_template.md`, and this task record. No code, workflow, protected file, or product spec is touched. `.github/ISSUE_TEMPLATE/engineering-task.md` was in the rule-5 grep scope but needed no change: it names no deploy job, and its peer-reviewer line is already the current-lane text.
- The review-lane switchover from #488 (review Claude-authored work with Claude; Codex author to Claude) is excluded by owner decision 2026-09-02. #488 is to be closed with a pointer to this PR (not closed by this branch).
