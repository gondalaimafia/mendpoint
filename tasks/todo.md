...
### 2026-08-26 ReGauge Mission artifact recovery replacement

- [x] Preserve stale PR #460 and transplant its complete reviewed recovery series onto a fresh `origin/main` branch.
- [x] Enumerate and review bounded input, persistence, replay/adoption, and tenant/principal authority paths together before the first CI retry.
- [x] Prove the exact 64 MiB acceptance boundary, greater-than-64 MiB fail-closed behavior, and readiness recovery across production, transfer, publication, and storage consumers.
- [ ] Add the current replacement PR self-declaration to the production-closure matrix without changing protected authority or production configuration.
- [x] Run focused ReGauge recovery suites, every affected workspace and scripts typecheck, full GA checks, optimized build, dependency audit, and strict diff/secret scans.
- [ ] Refetch and reconcile current `origin/main`, push the new Codex branch, and open a replacement PR that supersedes #460 only after it lands.
- [ ] Do not merge, deploy, rotate authority, or modify production state from this task.

#### Review

The fresh-main restack applies the complete 14-commit recovery series without conflict and leaves stale PR #460 untouched. The full-chain review found one remaining P1 family before CI: API legacy adoption and worker-local recovery could load historical candidate or execution evidence before applying the shared 64 MiB ceiling. The same unsafe consumer pattern was reachable from checkpoint publication, candidate replay, and recipe evidence replay. All five paths now validate regular-file identity, single-link ownership, a nonzero bounded size, and stable identity and size across the read. Sparse 64 MiB plus one byte regressions prove the API and worker paths fail before publication or command replay, while existing checkpoint and cutover tests prove the exact 64 MiB acceptance boundary and greater-than-ceiling rejection.

The corrected 13-file focused recovery matrix passes 222 tests, including the exact-boundary checkpoint and transfer cases. An advisory-observer assertion failed once only under the first parallel widened run and passed in isolation and in the complete focused rerun without source changes, so no timeout or unrelated behavior was changed. Full workspace and scripts typecheck, full `ga:check`, optimized production build, production dependency audit with zero vulnerabilities, strict diff validation, and the delta secret scan pass. Self-review of the complete main delta found no unresolved P0, P1, or P2 issue. PR creation and the exact closure-matrix self-declaration remain intentionally held until #495 lands so its protected open-PR snapshot is not invalidated.
