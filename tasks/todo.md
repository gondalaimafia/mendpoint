...
- Full-chain ReGauge deployment audit found a second P1 before push: the global release contract hard-coded Fettler, so a current ReGauge image would still advertise Fettler and keep its campaign capability experimental. Release metadata and feature classification are now profile aware, with canonical `Regauge` identity and `transformer_bsg_campaigns` GA only under `regauge_production`; Fettler behavior is unchanged. The protected workflow now rejects the coordinator unless `/version` and `/ready` prove that identity and capability, then proves exactly one current-revision coordinator on the retained volume and one current-revision volume-free worker. The RED run failed all three missing contracts; the GREEN run passes 68 focused tests, both workflow parses, and the full workspace typecheck.
