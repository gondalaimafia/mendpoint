...
- A later paid Fettler lease no longer requires an earlier cost row. Its fallback lineage links only the latest earlier attempt with a positive authenticated cost, so an uncharged first lease cannot wedge or misstate the retry.
...
- Hostile verification passes 40 tests across database, Worker, and API suites. Database, Worker, Transformer, and API typechecks pass, and diff integrity is clean. No push, merge, deployment, provider call, settlement rewrite, requirement promotion, or public-claim promotion occurred.
