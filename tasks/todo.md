...
- [x] Align the repository profile-intent binding with fail-closed live configuration readiness.
- [x] Run the focused workflow suite, YAML parse, configuration check, activation regression, and strict diff review.

### Review

The replacement is based on exact `origin/main` revision `7f74e609` and does not modify production closure authority policy, ledger, rotation, or matrix files. RED first: the focused suite failed two assertions because the old gate had no protected Fly authority and recorded no durable non-success result; a second RED run failed two assertions because both token-scope checks masked producer failures behind process substitution. Review RED: four workflow assertions proved that the protected preflight lacked a default-branch guard, did not distinguish the workflow revision from the live release revision, and still accepted substring-shaped output; four terminal-result cases failed before the exact verifier existed. GREEN: all seven focused workflow and producer-gate regressions pass. The two prefix-count terminal-result cases pass locally; the malformed and valid jq cases are retained but skip on this Windows host because `jq` is absent, and will execute on the Ubuntu workflow runner. The workflow and every embedded shell block parse, the configuration and action-pin gates pass, the scripts TypeScript project passes, and strict diff checking is clean. Eligibility now requires operator intent, live customer profile, and exact `MENDPOINT_RELEASE_REVISION == GITHUB_SHA`; inactive evidence retains both revisions without conflating them. A successful result is written only after exactly one uniquely prefixed compact JSON record passes schema, digest, and publication validation. `ME-ENT-007` remains partial pending a production backup provider, approved cross-region recovery infrastructure, and a real production regional failover drill.
...
Configuration-readiness correction: the profile-intent binding remained `optional_gated` after the workflow began treating absence as indeterminate authority. A live-compatible regression now loads the real manifest, verifies repository scope, and proves an absent binding produces `CONFIG_MISSING` with no gated-absence result. Read-only GitHub API evidence confirmed the variable exists at repository scope and has no environment-scoped duplicate.
