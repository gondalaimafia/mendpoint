...
- After rebasing onto current main `94185f82`, the combined database initializer preserves both the current secrets lifecycle migrations and the governed audit schema. The 31 focused API, worker, contract, and database tests, full workspace typecheck, optimized 64-route production build, every GA gate, zero-vulnerability production audit, and diff integrity pass on the rebased tree.
