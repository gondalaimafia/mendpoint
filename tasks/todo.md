...
## Protected Fettler delegated pull request vertical: 2026-08-18

Objective: turn the existing acceptance evaluator and authority readers into one protected, production-callable three-trial workflow. The ordinary pull request evaluation lane remains deterministic and reports production delegation as not measured. Only an exact protected run may publish an accepted result.

- [x] Re-audit exact main, current GitHub and Fly authority, callable runtime seams, evidence storage, and current protected workflows.
- [x] Extend exact GitHub observation and durable persistence with revision-bound changed paths, exact remote Git tree identity, exact matching draft count, and repository/install binding.
- [ ] Add an independent verifier operation that derives fail-to-pass and pass-to-pass evidence under a distinct principal, exact code digest, sandbox backend, source, candidate, command, logs, and timing authority.
- [ ] Add a transactional delegated trial assembler that accepts stable IDs only, re-reads every durable source and CI content-addressed object, invokes signed retain-exact cleanup first, derives all claims, signs the trial, and publishes one immutable bundle, evidence record, and event with stable replay semantics.
- [ ] Add signed contract, authenticated workflow-run receipt, pricing-table artifact, proof manifest, and stored acceptance-report authorities. Reject caller-selected clocks, unsigned reports, and unbound trust policy.
- [ ] Change production reporting to load and reverify one stored acceptance report. Never accept a caller-created DelegatedPrAcceptanceReport as production evidence.
- [ ] Add a protected Fettler workflow and orchestration command that pins one tenant, repository, installation, base revision, model route, budgets, verifier policy, and allowed paths; runs exactly three sequential jobs; requires OIDC human approval for each candidate; permits at most one open draft; always performs close plus retained-exact-head cleanup; and has no merge or deploy permission.
- [x] Close the approval audit timestamp provenance gap by binding the audit row timestamp to reviewedAt.
- [ ] Bind pricingDigest to the exact content-addressed pricing artifact used for metering.
- [ ] Run red-first focused tests, affected package suites and typechecks, complete repository tests, production build, GA and security gates, container/browser checks, and diff integrity.
- [ ] Publish through protected pull request only after strict P0/P1 review. Then reauthenticate GitHub, restore the suspended canary infrastructure without deleting the existing volume, verify all protected bindings, and execute the bounded three-trial run.

Acceptance: three unique trials use live model receipts, real GitHub App draft delivery, active OIDC human membership, independent fail-to-pass and pass-to-pass verification, exact token/cost/latency accounting, signed cleanup, and one immutable authority bundle each. The final stored report is accepted only when all three trials pass, all trial pull requests are closed unmerged with exact heads retained for rollback, no trial branch is merged or deployed, and the current production revision and authority remain exact. Missing credentials, suspended infrastructure, incomplete evidence, replay drift, stale proof time, or a failed trial leaves the result rejected and performs no further remote mutation.

Review before implementation: exact main `a69526ca5ecd0c5accd97e92cd8a64b7eb8d7361` is clean and CI-green. Existing queue, model, approval, draft delivery, cleanup, stored-trial reader, and evaluator components pass their focused suites. The missing product is orchestration and authoritative evidence production: no non-test trial writer, cleanup caller, independent verifier producer, stored report admission path, or protected three-trial workflow exists. The dedicated ReGauge Fly app is suspended at zero machines after SJC volume placement failures, and current GitHub CLI authentication is invalid, so live activation remains fail closed until source work is green and external authority is revalidated.

First implementation slice: the exact GitHub observer now optionally derives the immutable remote delivery evidence needed by the protected proof. It requires the exact App installation and repository scope before token use, exactly one pagination-complete open draft for the pinned branch and revisions, a comparison result bound to the exact base and head commits, a complete changed-path set including both sides of renames, and the exact head commit's Git tree SHA. The approved candidate keeps its separate content-tree digest; the evaluator never treats a Git object SHA as the local content digest. Delivery audit, CI observation, stored-trial authority, and acceptance evaluation must agree on the durable cycle and delivery IDs, repository, installation, paths, draft count, feedback state, and remote tree SHA. Incomplete pagination, duplicate or unsafe paths, comparison drift, non-null feedback on a successful observation, or more than one matching draft fails closed. The approval reader now also requires the chained audit row timestamp to equal the sealed review time; the API supplies that exact timestamp to the audit chain. Red-first tests failed on the missing GitHub boundaries, the cross-component repository/tree mismatch, the durable delivery-audit tree mismatch, the wrong-cycle/delivery/feedback joins, and the approval timestamp mismatch. GitHub passes 181 of 181, Worker 340 of 340, Eval 168 of 168, DB 281 of 281, and API 399 of 399. The full repository run was green except one Windows `EPERM` rename in the disaster-recovery suite; that exact test and the complete Ops suite then passed immediately in isolation, and all scripts pass 53 of 53. Every workspace and script typechecks, the optimized production build passes, GA governance checks pass, the production dependency audit reports zero vulnerabilities, and diff integrity passes. A local Chromium matrix checked the home page plus canonical Fettler and Regauge documentation at 1440 by 900 and 375 by 812: all six combinations returned 200 with zero horizontal overflow, console errors, failed requests, or serious and critical Axe violations. Independent strict review found no remaining P0 or P1 in this slice. Containerized deployment and crash-recovery evidence remains a protected pull-request CI gate.
