...
- Exact-head review repair: terminal reviewer withdrawal now requires a human-authored retraction whose Mission event exactly binds the actor, decision, status, scope, predecessor, and timestamp. A service principal can still record a generic Mission retraction, but the reviewer replacement path fails closed with `reviewer_directive_authority_invalid` instead of treating it as permanent human authority. The new adversarial regression raises the focused total to 59 passing tests; database typecheck remains green.
